...
| `docker/pc2-deploy-lib.mjs`                                                                       | Shared PC2-deploy logic — reads `PC2_SSH_HOST`/`PC2_REPO_PATH` from `.env`, SSHes in, and runs `docker compose pull` + `docker compose up -d` for the given services (or every service if none given). Handles the macOS non-interactive-SSH Docker Hub credential workaround automatically (see below) so you don't need to run it by hand.                                                                                                                                            |
