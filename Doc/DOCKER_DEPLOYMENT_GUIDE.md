...
3. Forward these ports on your router to PC2's static LAN IP: **80, 5000–5010, 8081, 8082** (see "Known limitations" below for why every backend port needs forwarding, not just the Gateway's).
...
- **Every backend service's port is published**, not just the Gateway's. The frontend's `environment.docker.ts` calls each service directly on its own port (mirroring how `environment.ts` works in local dev), so `docker-compose.yml` publishes ports 5000–5010, not only 5000. If you forward ports on your router, you're exposing all of them, not just the Gateway — routing every frontend call through the Gateway only would require a frontend HTTP-client refactor, not attempted here.
