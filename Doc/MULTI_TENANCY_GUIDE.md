...
- **Resilient resubscription (August 2026):** both `TenantProvisioningListenerService<TContext>` (Layer 3, `tenant:provisioned`) and `NasheedTenantConfigUpdatedListenerService` (`tenant:updated`) wrap their initial `SubscribeAsync` call in a shared exponential-backoff retry (`SubscribeWithRetryAsync`) — 2s initial delay, doubling up to a 30s cap, retried indefinitely until the subscription succeeds or the host stops. Previously a failed first subscribe (Redis unreachable at that exact startup moment) either crashed the host outright or left the listener permanently dead until a manual restart; now it keeps retrying in the background and self-heals the moment Redis becomes reachable, no restart needed.
- **`NasheedTenantLoaderService`'s startup load is also now self-healing (August 2026):** the original fast retry loop (12 attempts × 5s) that runs while `NasheedIngestionWorker` waits for the tenant to become ready still exists, but exhausting it no longer means permanent failure — a new `RetryTenantLoadInBackgroundAsync` takes over and retries every `FallbackRetryInterval` (1 minute) indefinitely until the tenant loads successfully. Before this, a Tenant Service outage longer than the fast loop's ~60-second window at Nasheed startup stranded the ingestion worker until a manual restart; now it recovers on its own once Tenant Service is back.
