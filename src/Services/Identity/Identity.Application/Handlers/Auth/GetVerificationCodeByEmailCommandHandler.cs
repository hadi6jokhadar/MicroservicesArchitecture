...
public class GetVerificationCodeByEmailCommandHandler : IRequestHandler<GetVerificationCodeByEmailCommand, VerificationCodeResponseDto>
{
    ...
    private readonly ILogger<GetVerificationCodeByEmailCommandHandler> _logger;
    ...
    public GetVerificationCodeByEmailCommandHandler(
        ...
        ILogger<GetVerificationCodeByEmailCommandHandler> logger)
    {
    ...
    public async Task<VerificationCodeResponseDto> Handle(GetVerificationCodeByEmailCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Check if email exists. Always report success even when it doesn't (same
            // anti-enumeration approach as ForgetPasswordCommandHandler) so a caller can't use
            // this endpoint to probe which emails are registered.
        ...
        {
            _logger.LogError(ex, "An error occurred while getting verification code for email: {Email}", request.Email);
