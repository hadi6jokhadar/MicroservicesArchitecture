---
description: Guide and workflow on how to write and use ILogger in handlers or other classes in the application.
---
...
1.  **Dependency Injection**: Inject `ILogger<T>` via the constructor, where `T` is the type of the current class.
...
2.  **Structured Logging**: Use structured logging rather than string interpolation when passing variables to the log message. This ensures variables can be indexed by log aggregators.
